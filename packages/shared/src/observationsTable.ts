...
const OBSERVATION_TYPE_DISPLAY_MAP: Record<string, string> = {
  AGENT: "智能体",
  CHAIN: "链路步骤",
  EMBEDDING: "向量嵌入",
  EVALUATOR: "评测器",
...
const localizeObservationTypeOptions = (options: Array<SingleValueOption>) =>
  options.map((option) => ({
    ...option,
    displayValue: OBSERVATION_TYPE_DISPLAY_MAP[option.value] ?? option.value,
  }));
  ...
  return observationsTableCols.map((col) => {
    ...
    if (col.id === "type") {
      return formatColumnOptions(
        col,
        localizeObservationTypeOptions(options?.type ?? []),
      );
    }
    if (col.id === "toolNames") {
