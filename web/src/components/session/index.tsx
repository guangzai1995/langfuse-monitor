...
const SESSION_DETAIL_SYSTEM_PRESETS: SystemFilterPreset[] = [
  {
    ...
    name: "每条 Trace 的最后一个 Generation",
    description: "仅显示每条 Trace 中最后一个 Generation 节点",
    filters: [
      {
        column: "type",
        type: "stringOptions",
  ...
  },
  {
    ...
    name: "根观测节点",
    description: "仅显示每条 Trace 的根观测节点",
    filters: [
      {
        column: "positionInTrace",
        type: "positionInTrace",
        ...
        <Popover modal>
          <PopoverTrigger asChild>
            <Button variant="outline" size="sm" className="mt-0.5">
              另外 {remainingUsers.length} 位用户
            </Button>
          </PopoverTrigger>
          <PopoverContent className="w-[300px]">
            <Label className="text-base">会话用户</Label>
                  ...
                  .map((userId: string) => (
                    ...
                    >
                      <Badge className="max-w-[260px]">
                        <span className="truncate">用户 ID：{userId}</span>
                        <ExternalLinkIcon className="ml-1 h-3 w-3" />
                      </Badge>
                    </Link>
                  ))}
...
export const SessionPage: React.FC<{
  ...
  if (session.error?.data?.code === "UNAUTHORIZED")
    return <ErrorPage message="你无权访问此会话。" />;
      ...
      <div className="flex h-full flex-col overflow-auto">
        <div className="bg-background sticky top-0 z-40 flex flex-wrap gap-2 border-b p-4">
          ...
          <Badge variant="outline">
            Trace 总数：{session.data?.traces.length}
          </Badge>
          {session.data && (
            <Badge variant="outline">
