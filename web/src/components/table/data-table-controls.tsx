...
import { type FilterState, type SingleValueOption } from "@langfuse/shared";
        ...
        <div className="flex items-center gap-1">
          {queryFilter.isFiltered && (
            <Tooltip>
              ...
              <TooltipContent>清空全部筛选条件</TooltipContent>
            </Tooltip>
          )}
...
export function CategoricalFacet({
  ...
  const getOptionDisplayValue = useCallback((option: SingleValueOption) => {
    return option.displayValue ?? (option.value === "" ? "(empty)" : option.value);
  }, []);
            ...
            */}
            ...
            {loading ? (
              ...
              <div className="text-muted-foreground py-1 text-xs">
                {filterKey === "sessionId" ? (
                  <span>
                    会话可以把多条 Trace 归并到一起，便于追踪多步骤工作流。{" "}
                    <a
                  ...
                  <span>当前时间范围内未找到链路名称。</span>
                ) : filterKey === "tags" ? (
                  <span>
                    标签可以帮助你按照自定义类别筛选链路，例如功能开关。{" "}
                    <a
              ...
              <>
                  ...
                  <>
                    {visibleOptions.map((option) => (
                      <FilterValueCheckbox
                        ...
                        label={getOptionDisplayValue(option)}
                ...
                options[0]?.value.toLowerCase() === "default" ? (
                  <div className="text-muted-foreground mt-2 px-2 text-xs">
                    环境可以帮助你区分不同上下文中的链路，例如生产环境或预发布环境。{" "}
                    <a
...
function FilterModeTabs({ mode, onModeChange }: FilterModeTabsProps) {
  return (
    <div className="@container mb-2 flex flex-wrap items-center gap-1.5 px-4">
      <span className="text-muted-foreground/80 text-[10px]">模式：</span>
