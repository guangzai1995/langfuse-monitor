...
function processOptions(options: (string | SingleValueOption)[]): {
  normalizedOptions: SingleValueOption[];
  values: string[];
  counts: Map<string, number>;
} {
  const normalizedOptions: SingleValueOption[] = [];
  const values: string[] = [];
  const counts = new Map<string, number>();

  for (const opt of options) {
    if (typeof opt === "string") {
      normalizedOptions.push({ value: opt });
      values.push(opt);
  ...
  const filters: UIFilter[] = useMemo((): UIFilter[] => {
    ...
    return config.facets
      .map((facet): UIFilter | null => {
        ...
        const { normalizedOptions, values: availableValues, counts } = Array.isArray(
          availableValuesWithOptions,
        )
          ? processOptions(availableValuesWithOptions)
          : { normalizedOptions: [], values: [], counts: EMPTY_MAP };
